/* eig_par_thr.c
 * Parallel PCA using MPI for data/covariance/projection and OpenMP on root
 * for the Jacobi eigensolver.
 */
...
int main(int argc, char **argv) {
	...
	int k = atoi(argv[2]);
	const char *outp_prefix = (argc >= 4) ? argv[3] : "results/eig_par_thr";
	size_t global_n = 0;
	...
	if (!C_local) {
		...
		if (total_n < 2) {
			...
			if (!Vbuf) {
				...
				rc = jacobi_eigen_parallel_gather_root_omp(C_local->data, d_cov, eigvals, Vbuf, MPI_COMM_WORLD, 0, jacobi_tol, jacobi_maxiter);
				if (rc == 0) {
					...
					if (!eigvecs) {
						...
						if (!Vmat) {
							...
							if (!proj_local || matrix_dgemm(1.0, X_local, Vmat, 0.0, proj_local) != 0) rc = -1;
							matrix_free(Vmat);
						}
					}
				}
	...
	if (gather_projection) {
		if (eig_debug && rank == 0) fprintf(stderr, "[eig_par_thr] gathering projected data to root\n");
		...
		if (rank == 0) {
			...
			if (projection_global) {
				matrix_write_csv("results/eig_par_thr_projection.csv", projection_global);
